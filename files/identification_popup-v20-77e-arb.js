...
var country_ip = '';
var ip_country = '';
var UrlPri2 = webAddress.match(/^dev/)?"dev1-":(webAddress.match(/^stg/))?"stg1-":"";
var UrlPri = webAddress.match(/^dev/)?"dev-":(webAddress.match(/^stg/))?"stg-":"";
var country_ip_iden = '';
var country_iso_iden = '';
var country_nm_iden = '';
var identification_currect_version = 18;

$(document).ready(function(){
    setTimeout(function(){callIdentificationPopup()},3000);
});
...
function callIdentificationPopup(){
    ...
    if(rspv == 2 || rspv == 4 || rspv == 6){
        ...
        if(c_imesh == '' || c_imesh==null){
           ...
           setTimeout(function(){identify_Banner()},1000);
...
{   
    if(typeof(mob_num) !== "undefined" && mob_num != ''){
...
function get_DomainURL()
{
    ...
    else if(window.location.hostname.match(/stg-|stg\./ig))
    {
        mapi_domain = 'stg-mapi.indiamart.com';
    }
    return mapi_domain;
...
function userDetailsAutoFetchforidenty(user_name)
{
    ...
    var geoloc = readCookie('GeoLoc');
    var lat_val = (geoloc !='') ? getparamVal(geoloc, "lt") : '';
    var long_val = (geoloc !='') ? getparamVal(geoloc, "lg") : '';
...
function callIdentifyAfterGettingIP(user_name,params_request){
    //Get IP here and then call Identify
...
function callIdentify(user_name,params_request){
    ...
    $.ajax({
url: "//"+UrlPri2+"login.indiamart.com/user/identify/",
data: params_request,
type: 'POST',
    ...
    if(jsonResult != undefined && jsonResult["code"] == '200')
    {
        ...
        var imeshVisrFlag = 0;
            ...
            success: function (jsonResult1){
                            ...
                            if(name != '' && email != '' && city != ''){var flag = 4;}else{var flag = 3};
                            if(!(v4iil == "" || v4iil == null))
                            {
                                if(imesh.phcc == '91')
                                {
                                    ...
                                    /*identfy_verify(1);*/$("html").css("overflow","auto"); callToIdentifiedQ(); $("#idfpclose, #identifiedForm,  #indicatorLoader, #welComp").hide(); $('#maskctl').hide(); $('#identyfy_usr_ctl').hide(); $("#overlay_s").css("display","none");
                                    $("html").css("overflow","auto");
                                    $('#IdentifiedPopUpHTML').html('');
                                }
                            }
                            else if(imesh.phcc == '91' && imesh.uv != 'V')
                            {     
                                $("#overlay_s").css("display","block");
                                $('#idn').css({"width":"100%","height":"100%"});
                                send_otp(glUserId,modid,user_name,ph_country,attributeId,cn_iso,'2',flag,screen);
                            }
                            else
                            {
                                //identfy_verify(1);
                                $("html").css("overflow","auto"); callToIdentifiedQ(); $("#idfpclose, #identifiedForm,  #indicatorLoader, #welComp").hide(); $('#maskctl').hide(); $('#identyfy_usr_ctl').hide(); $("#overlay_s").css("display","none");
                                $("html").css("overflow","auto");
                                $('#IdentifiedPopUpHTML').html('');
                                if(modid=='IMHOME'){
                                    window.location.reload();
                            ...
                            if(spv1 == 2)
                                {
//                                    _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_submit_2nd',modid, 0, false]);  
                                    yandex_impression_identification('Identification_login_popup_submit_2nd_'+modid);
                            }
                            if(spv1 == 4)
                                 {
//                                     _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_submit_4th',modid, 0, false]);  
                                     yandex_impression_identification('Identification_login_popup_submit_4th_'+modid);
                            }
                            if(spv1 == 6)
                                 {
//                                     _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_submit_6th',modid, 0, false]);
                                     yandex_impression_identification('Identification_login_popup_submit_6th_'+modid);
                            }
                            if(spv1 == 7)
                                 {
//                                     _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_submit_7th',modid, 0, false]);
                                     yandex_impression_identification('Identification_login_popup_submit_7th_'+modid);
                            }
    ...
    {   
        ...
        var mob_box = document.getElementById('mbl_idn');
        var err_div = document.getElementById('err-msg-mbl-ctl');
        if(jsonResult["message"].match("observed suspicious activity in your")){
...
{      
        ...
        $("#thankuMsg").show();
        $('#logintoidentify').css({"visibility":"hidden"});
...
function emailProper(useremail1)
{
                ...
                else if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,})+$/.test(useremail1))) {
                    $('#starMandatory').css("display","block");
...
{
//    _gaq.push(['_trackEvent','Identification_login_popup','Click_to_verify_IM_details_NoGoogle', modid, 0, false]);
...
function attachSignin(element) 
{
    auth2.attachClickHandler(element, {},
        ...
        function(googleUser) {
//            _gaq.push(['_trackEvent','Identification_login_popup','Click_On_Connect_With_Google_Button', modid, 0, false]);
        ...
        function(error) 
        {
//          _gaq.push(['_trackEvent','Identification_login_popup','Google_login_closed', modid, 0, false]);
...
function closePop()
{
    if (googuser == 1)
    {
//      _gaq.push(['_trackEvent','Identification_login_popup','Google_IM_Email_Duplicate_Close', modid, 0, false]);
        yandex_impression_identification('Google_IM_Email_Duplicate_Close_'+modid);
    }
    else
    {
//      _gaq.push(['_trackEvent','Identification_login_popup','Continue_With_Google_Close', modid, 0, false]);
...
function validateForm_userName(mob_box,errdiv) 
{
    ...
    var mobVal = mob_box.value;
        ...
        if($('#countryIso').val()=="IN")
        {
            ...
            else if((/^[0-9-()\[\]\s]{0,30}$/).test(mobVal))
            {
                    var rmSpeChar= mobVal.replace(/\-|\s|\[|\]|\(|\)/ig,'');
                    var mobValFilter = /^(?:(?:\+|0{0,2})(91|910)(\s*[\-]\s*)?|[0]?)?[16789]\d{9}$/;//for checking mobile no. starting with 6,7,8,9
                    var validMob=mobValFilter.test(mobVal);
                    var cnt = rmSpeChar.length;
                    if(cnt>10){rmSpeChar = mobVal.replace(/^((91){0,1}0{0,})/g,'');}
                    ...
                    else if(($("#MobCCode").val() == '+91' || $("#MobCCode").val() == '91' || $("#countryIso").val() == 'IN') && (validMob == false))
                    {
                            invalidmsg_ctl(mob_box,"Mobile Number should start with 1,6,7,8 or 9",err_div);
                            $('#logintoidentify').removeClass("load_s bgnn");
                            return false;   
                    }
                    else if(rmSpeChar.length > 20)
                    {
                    ...
                    else 
                    {
                            ...
                            $("#"+errdiv).html("");
                            $('#logintoidentify').css({"visibility":"show"});
        ...
        else
        {
            ...
            if((mobVal == '') || (mobVal.length == 0))
            {
                invalidmsg_ctl(mob_box,"Please enter your Email ID",err_div);
                $('#logintoidentify').removeClass("load_s bgnn");
                return false;      
            }
        ...
        $.ajax({
            ...
            success: function ( jsonResult ){
              ...
              if(jsonResult['code']==200){
                ...
                if((resultSet != undefined) && (resultSet !='')){
                  ...
                  if(multi_flag==1){
                    ...
                    else
                    {
                        ...
                        //showmobverifyform(glUserId,modid,username,ph_country,'121',iso,flag,'IDEN','');
                    ...
                    verifyEmailViaLWGIden(gusremail,glid);
                      ...
                      {
                          //truecaller_Ver(glUserId,modid,username,ph_country,'121',iso,'2',flag);
                          //(glUserId,modid,username,ph_country,'121',iso,flag,'IDEN','');
                          send_otp(glUserId,modid,username,ph_country,'121',iso,'2',1,'IDEN');
                          $("#loadinglwg").hide();
                          getLoginStringv1();
                          $("#overlay_s").show();
                ...
                if(jsonResult["code"] == '204'){
                                if(jsonResult["message"] == 'ISO MisMatch'){
                                    //invalidMsgLogin("You seem to be from India. Select India as Country.");
                                  ...
                                  var msg = jsonResult["message"];
                                    ...
                                    $("#logintoidentify").removeClass('load_s bgnn') ;
                                    $("#spn").css('display','none');
                                    $("#err-msg-mbl-ctl").css({'text-align':'center','margin-left':'0px','width':'auto'});
                                }
                                else{ 
...
function getIdentifiedPopUpHTMLForm1()
{
   ...
   setTimeout(function(){
     ...
     $("#IdentifiedPopUpHTML").html("");
        ...
        if(glmodid == 'MY' || glmodid == 'SELLERMY'  || glmodid == 'TDR'  || glmodid == 'BIGBUYER' || glmodid == 'CTL' || webAddress == '/seller/' || iso_by_ip != 'IN') 
            {  
            $("#IdentifiedPopUpHTML").html("");
            identifiedPopName = "";
            is_form_open=0;
...
//          p_nm = p_nm.charAt(0).toUpperCase() + p_nm.slice(1);
            ...
            else {p_new_msg = 'Login to connect with '+p_cnt+' verified suppliers';} 
            }
            else{
            if(spv1 == 4){
            ...
            p_nm = ''+p_nm+' in '+p_city+'';
            p_new_msg = 'Choose from over 42 lakh suppliers ';
            }
            ...
            {
             $('#identyfy_usr_ctl').css({"width":"520px"});
             $('#identyfy_usr_ctl').css({"height":"420px"});
             $('#lbl1').css({"padding-left":"182px"});
             $(".lookingfor").css({"font-size":"20px"});
             $('#ghead').css({"font-size":"20px"});
             $('#countrySuggesterIdenPop dt span.value').css({"height": "30px"});
            }
            else if(p_nm.length > 52)
            {
            $('#identyfy_usr_ctl').css({"width":"520px"});
             $('#identyfy_usr_ctl').css({"height":"420px"});
             $('#lbl1').css({"padding-left":"182px"});
            ...
            {
            $('#identyfy_usr_ctl').css({"height":"400px"});   
            }
        }
        else if((glmodid == 'ETO' && location.href.match(/buyersearch.mp/)))
            {
            if(document.getElementById("blSearchHeader") != null){
            var div = document.getElementById('blSearchHeader').innerText;
            }
            ...
            {
               $('#identyfy_usr_ctl').css({"width":"540px"}); 
            }
            if(spv1 == 4 || spv1 == 6 || spv1 == 7)
            {
            $("#idfpclose").hide();    
            ...
            $('#welcomeMsg').css({"line-height":"56px"});}
            0}
            if(glmodid == 'ETO'){
                if(glmodid == 'ETO'){
                    $('#countrySuggesterIdenPop dt span.value').css({"bottom": "2px !important","font-size": "14px !important"});
                }
            }
            ...
            success: function ( jsonResult ){
              ...
              if(jsonResult['code']==200){
                ...
                if(msg == 'Unique Email found in Primary Email'){
                    ...
                    //set Imesh visitor and v4iilx cookies here to give full login
                    //var resultSet=jsonResult.LOGIN_DATA.DataCookie;
                    verifyEmailViaLWGIden(gusremail,glid);
                    //update fname via google login if it is blank in DB
...
{      
        ...
        if(spv1 == 2){
//            _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_close_2nd',modid, 0, 0]); 
            yandex_impression_identification('Identification_login_popup_close_2nd_'+modid);
        }
        if(spv1 == 4){
//            _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_close_4th',modid, 0, true]); 
            yandex_impression_identification('Identification_login_popup_close_4th_'+modid);
        }
        if(spv1 == 6){
//            _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_close_6th',modid, 0, true]); 
            yandex_impression_identification('Identification_login_popup_close_6th_'+modid);
        }
        if(spv1 == 7){
//            _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_close_7th',modid, 0, true]);
...
function changePopUpInput(identifiedPopName,flag) 
{
    ...
    var err_div = document.getElementById('err-msg-mbl-ctl');
     ...
     if(glmodid == 'FCP' || glmodid == 'MDC'){
        var validnE = '<span class="check_icon" style="display:inline-block;float:none;"></span>Your Email is safe with us';
     ...
     if($("#countryIso").val() == ''){
        ...
        if ( iso == arr[i] ){
            ...
            if ( isEuroCountry && $("#IdentifiedPopUpHTML").html()!='' )
                ...
                else if($("#IdentifiedPopUpHTML").html()!='')
                {
                document.getElementById("logintoidentify").disabled = false;
                $("#logintoidentify").css("background-color","#00a699");
                }
            $('#countrySuggesterIdenPop dt span.value').css("display", "none");
            validmsg_ctl(mob_box,err_div,flag);
            $('#identifiedForm #mbl_idn').css({"width":"306px"});
            $('#identifiedForm #mbl_idn').css({"padding-left":"47px"});
            if(glmodid == 'DIR'){
                $('#identifiedForm #lbl1').text("Enter your Email ID");
                if(typeof LWG_call != undefined && LWG_call == 1){
                    $('#identifiedForm #lbl1').html('Mobile Number <span style="color:blue;font-size:11px;">(Optional)</span>');
                }
            }
            else
            {
                $('#identifiedForm #mbl_idn').attr("placeholder", "Enter your Email ID");
                if(typeof LWG_call != undefined && LWG_call == 1){
                    $('#mbl_idn').attr("placeholder", "Enter your Mobile Number");
                }
            }
        ...
        else
        {
           ...
           fieldFlag = 1;
            ...
            $("#logintoidentify").css("background-color","#00a699");
            validmsg_ctl(mob_box,err_div,flag);
            $('#countrySuggesterIdenPop dt span.value').css({"display":"inline-block","height":"32px","padding":"7px 4px 2px 3px"});
            if(glmodid == 'ETO'){
                $('#countrySuggesterIdenPop dt span.value').css({"display":"inline-block","height":"32px","padding":"7px 4px 2px 3px","box-sizing": "unset","bottom": "2px !important","left": "10px","font-size": "14px !important"});
                $('.cntry_drpdwn2 .value').css({"bottom":"2px !important","font-size":"14px !important"});
            }
            //$('#countrySuggesterIdenPop dt span.value').css("height", "30px");
            //$('#countrySuggesterIdenPop dt span.value').css("padding-top", "8px!important");
            if(glmodid == 'PRODDTL')
                $('#countrySuggesterIdenPop dt span.value').css("height", "28px");
            $('#identifiedForm #mbl_idn').css({"width":"253px"});
            $('#identifiedForm #mbl_idn').css({"padding-left":"100px"});
            if(glmodid == 'DIR'){
                $('#identifiedForm #lbl1').text("Enter your mobile number");
                if(typeof LWG_call != undefined && LWG_call == 1){
                    $('#identifiedForm #lbl1').html('Mobile Number <span style="color:red;">*</span>');
                }
            }
            else
            {
...
function countryFlagSuggesterIdentifiedPopup()
{
    ...
    var country_sugg = new Suggester({"type":"isd","element":"countrySuggesterIdenPop","onSelect":selectCountry1Pop, fields: "cname,iso,icon_order",autocompleteClass:"isd_class",displayFields:"cname,value",displaySeparator:'  +','defaultValue':country_code});
}
function identify_Banner()
{
            ...
            $("#identyfy_usr_ctl").fadeIn();
             ...
             var spv1 = getparamValIden(readCookie("sessid"), "spv");
            if (typeof(glmodid) != 'undefined') { modid=glmodid;}
            if(modid == 'PRODDTL'){ var Prod_var = window.enq_dsbl;}
            ...
            { 
                if(spv1 == 2){
//                    _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_displayed_2nd',modid, 0, true]);
                    yandex_impression_identification('Identification_login_popup_displayed_2nd_'+modid);
                }
                if(spv1 == 4){
//                    _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_displayed_4th',modid, 0, true]);
                    yandex_impression_identification('Identification_login_popup_displayed_4th_'+modid);
                }
                if(spv1 == 6){
//                    _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_displayed_6th',modid, 0, true]);
                    yandex_impression_identification('Identification_login_popup_displayed_6th_'+modid);
                }
                if(spv1 == 7){
//                    _gaq.push(['_trackEvent', 'Identification_login_popup','Identification_login_popup_displayed_7th',modid, 0, true]);
                    yandex_impression_identification('Identification_login_popup_displayed_7th_'+modid);
                }
            }
            
            $('.idfpclose').click(function () {$("#IdentifiedPopUpHTML").html("");//$('#maskctl').hide();$('#identyfy_usr_ctl').hide();$('#idn').hide();             
                is_form_open=0;
            });
